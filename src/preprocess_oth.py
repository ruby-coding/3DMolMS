...
def csv2arr(df, encoder):
    '''
    Converts a DataFrame containing SMILES and labels into structured NumPy arrays.
    Output format:
    [
        {'title': <str>, 'mol': <numpy array>, 'label': <int>, 'mol_meta': <numpy array>},
        {'title': <str>, 'mol': <numpy array>, 'label': <int>, 'mol_meta': <numpy array>},
        ...
    ]
    '''
    ...
    for idx, row in tqdm(df.iterrows(), total=df.shape[0]):
        ...
        good_conf, xyz_arr, atom_type = conformation_array(smiles, conf_type=encoder['conf_type'])
        ...
        # Pre-compute molecular metadata (similar to env_arr in AllCCS)
        mol = Chem.MolFromSmiles(smiles)
        if mol is not None:
            mol_mass = Descriptors.MolWt(mol)
            total_atomic_number = sum([atom.GetAtomicNum() for atom in mol.GetAtoms()])
            mol_meta = np.array([mol_mass, total_atomic_number])
        else:
            mol_meta = np.array([0.0, 0.0])

        # Store in the structured format (following AllCCS pattern)
        data.append({
            'title': f"Mol_{idx}",  # Generated molecule ID
            'mol': mol_arr,  # Molecular representation
            'label': np.array([label]).astype(np.int64),  # Classification label (consistent with CCS format)
            'mol_meta': mol_meta  # Molecular metadata (similar to env_arr)
        })

    return data


def mollama2arr(df, encoder):
    ...
    for idx, row in tqdm(df.iterrows(), total=df.shape[0]):
        ...
        embedding = row[[f'mol_embed_{i}' for i in range(2048)]].values.astype(np.float32)
        ...
        data.append({
            ...
            'label': np.array([row['labels']]).astype(np.int64),
...
if __name__ == "__main__": 
    ...
    if 'mollama' in args.dataset:
        # 1. Load data
        print('\n>>> Step 1: load the dataset;')
        df = pd.read_csv(os.path.join(args.raw_dir, 'mito_dys_mollama.csv'))
        df = df.dropna(subset=['smiles', 'labels'])
        ...
        # Save test data
        test_data = mollama2arr(test_df, config['encoding'])
        out_path = os.path.join(args.pkl_dir, 'mollama_{}_test.pkl'.format(
            config['encoding']['conf_type']))
        with open(out_path, 'wb') as f:
            pickle.dump(test_data, f)
            print('Save {}'.format(out_path))
